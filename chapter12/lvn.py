...
def form_blocks(body):
    ...
    for instr in body:
        if 'op' in instr: # An acutal instruction
...
# A Value uniquely represents a computatoin in terms of sub-values.
...
def last_writes(instrs):
    ...
    seen = set()
    #We reverse the list so the first seen will be the last def
    # in forward order
    ...
    for instr, last_write in zip(block, last_writes(block)):
        ...
        if 'dest' in instr:
            ...
            if last_write:
                ...
                # reused by another compuation in the feature (in case
